...
/*   Updated: 2026/02/18 10:00:53 by cdahne           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cast_ray(t_vars *vars, double beta, int ray_id, bool draw_map)
{
	...
	first_hor_intersec(vars, &wall_slice_hor, beta);
	first_ver_intersec(vars, &wall_slice_ver, beta);
	while (wall_slice_hor.touch == NOTOUCH)
		next_hor_intersec(vars, &wall_slice_hor, beta);
	while (wall_slice_ver.touch == NOTOUCH)
		next_ver_intersec(vars, &wall_slice_ver, beta);
