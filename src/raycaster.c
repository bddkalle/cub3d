...
t_touch	wall_info(t_vars *vars, float px, float py, float beta)
{
	t_touch	wall_slice;
...
void	draw_vertical_line(t_vars *vars, int ray_id, float dist, t_touch *touch)
{
	int		y;
...
void	cast_ray(t_vars *vars, float beta, int ray_id, bool draw_map)
{
	...
	wall_slice = wall_info(vars, ray_x, ray_y, beta);
	//draw_vertical_line(vars, ray_x, ray_y, ray_id, beta);
	draw_vertical_line(vars, ray_id, dist, &wall_slice);
}
